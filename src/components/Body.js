...
let timetable = new Array(126).fill(0);
...
export default class Body extends Component {
  ...
  pushToSchedule = () => {
    ...
    for (let x = 0; x < 126; x++) {
      schedules[len - 1].push(timetable[x]);
    }
  };
  ...
  whenCiIs4 = (idx) => {
    for (let i = 0; i < chosenCourse[idx].row.length; i++) {
      ...
      if (this.checkOverlap(timeidx, timelen) == false) {
        for (let j = 0; j < timelen; j++) {
          timetable[timeidx + j] = chosenCourse[idx].row[i];
        }
    ...
    if (lecArr.length > 0) {
            ...
            for (let j of semArr) {
              ...
              if (this.checkOverlap(sem_timeidx, sem_timelen) == false) {
                for (let k = 0; k < sem_timelen; k++) {
                  timetable[sem_timeidx + k] = chosenCourse[idx].row[j];
                }
                if (labArr.length > 0) {
                  ...
                  for (let jj of labArr) {
                    ...
                    if (this.checkOverlap(lab_timeidx, lab_timelen) == false) {
                      for (let k = 0; k < lab_timelen; k++) {
                        timetable[lab_timeidx + k] = chosenCourse[idx].row[jj];
                      }
                      if (idx == chosenCourse.length - 1) {
                        this.pushToSchedule();
                      } else {
                      ...
                      for (let k = 0; k < lab_timelen; k++) {
                        timetable[lab_timeidx + k] = 0;
                      }
                    }
                  }
                  for (let k = 0; k < sem_timelen; k++) {
                    timetable[sem_timeidx + k] = 0;
                  }
                } else {
      ...
      if (semArr.length > 0) {
        ...
        for (let i of semArr) {
          ...
          if (this.checkOverlap(sem_timeidx, sem_timelen) == false) {
            for (let k = 0; k < sem_timelen; k++) {
              timetable[sem_timeidx + k] = chosenCourse[idx].row[i];
            }
            if (idx == chosenCourse.length - 1) {
              this.pushToSchedule();
            } else {
        ...
        for (let i of lecArr) {
          ...
          if (this.checkOverlap(timeidx, timelen) == false) {
            ...
            if (semArr.length > 0) {
              ...
              for (let aa of labArr) {
                let lab_timeidx =
                  (Number(chosenCourse[idx].row[aa].weekday) - 1) * 18 +
                  Number(chosenCourse[idx].row[aa].timeid) -
                  1;
                let lab_timelen = Number(chosenCourse[idx].row[aa].durtime);
                if (this.checkOverlap(lab_timeidx, lab_timelen) == false) {
                  for (let k = 0; k < lab_timelen; k++) {
                    timetable[lab_timeidx + k] = chosenCourse[idx].row[aa];
                  }
                  if (idx == chosenCourse.length - 1) {
                    this.pushToSchedule();
                  } else {
